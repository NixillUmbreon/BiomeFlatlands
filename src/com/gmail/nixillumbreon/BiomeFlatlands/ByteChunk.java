...
public class ByteChunk {
	...
	public ByteChunk (World world, int chunkX, int chunkZ) {
		...
		this.blocks = new byte[height / 16][];
	}
	
	public void setBlock(int x, int y, int z, byte materialId) {
        if (blocks[y >> 4] == null) blocks[y >> 4] = new byte[4096];
        blocks[y >> 4][((y & 0xF) << 8) | (z << 4) | x] = materialId;
	}
	
	public byte getBlock(int x, int y, int z) {
		if (blocks[y >> 4] == null) return (byte)0;
		return blocks[y >> 4][((y & 0xF) << 8) | (z << 4) | x];
	}
	
	public void setLayer(int y, byte materialId) {
		if (blocks[y >> 4] == null)	blocks[y >> 4] = new byte[4096];
		Arrays.fill(blocks[y >> 4], (y & 0xF) << 8, (((y & 0xF) << 8) | (15 << 4) | 15) + 1, materialId);
	}
	
	public void setBlock(int x, int y, int z, Material mat) {
		setBlock(x, y, z, (byte)(mat.getId()));
	}
	public void setLayer(int y, Material mat) {
		setLayer(y, (byte)(mat.getId()));
	}
